...
function WebRTC(opts) {
    ...
    connection.on('enableChatList',function(cts) {
        //document.getElementById(element).value = cts;
        console.log('enableChatList params is :'+cts );
        var rooms = cts.split("|");
        if(rooms) {
            var roomStr;
            for(var r in rooms) {
                roomStr += "<input type='radio' value='" + r + "'>" + r + "</input>";
            }
            document.getElementById("blist").innerHTML = roomStr;
...
WebRTC.prototype.reqChatList = function () {
    this.connection.emit('showbundle');
};
